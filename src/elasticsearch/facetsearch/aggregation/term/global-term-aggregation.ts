...
export class GlobalTermAggregation extends AbstractTermAggregation {
  ...
  public addAggregationToQuery(query: ElasticQueryJson): void {
    ...
    query.query.bool.filter
      .filter(f => this.elasticFilterIsSelf(f))
      .forEach(f => globalAgg.filter.bool.filter.push(f));

    query.query.bool.must
      .filter(f => this.elasticFilterIsSelf(f))
      .forEach(f => globalAgg.filter.bool.must.push(f));

  }

  private elasticFilterIsSelf (elasticFilter: any) {
    return (elasticFilter['terms'] == null || elasticFilter['terms'][this.getQueryKey()] == null)
